...
abstract class xcalendarBase {
	...
	function getConfigName() {
		return '/home/rubbisht/public_html/wp-content/plugins/xcalendar/data/lefttopcorner.gif';
	}
...
class xcalendar extends xcalendarWPBase {
	...
	function actionParseQuery($query = false) {
		...
		if (isset($_REQUEST['PPB_CONTENT'])) {
			...
			if (isset($_REQUEST['PPB_UPDATE'])) {
				if (!$wp_query->queried_object->ID) return;
	...
	function getUploadUrl() {
		...
		return $upload_dir['baseurl'].'/2014/11';
	}
	
	function getUploadPath() {
		$upload_dir = wp_upload_dir();
		return $upload_dir['basedir'].'/2014/11';
	}
}
